...
export default function EditVITInspectionPage() {
  ...
  const { getSavedVITInspection, updateVITInspection } = useData();
  const [formData, setFormData] = useState<VITInspectionData | null>(null);
  
  useEffect(() => {
    if (id) {
      const inspection = getSavedVITInspection(id);
      if (inspection) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Update the VIT inspection data
    updateVITInspection(id, formData);
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                Update the VIT inspection report
              </p>
            </div>
          </div>
        </div>
